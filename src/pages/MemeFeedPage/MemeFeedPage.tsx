...
import { useAuthToken } from '../../contexts/authentication.tsx';
import { useMutation, useQuery } from '@tanstack/react-query';
import { GetMemeCommentsResponse, GetMemesResponse, GetUserByIdResponse } from '../../api/api.ts';
...
export const MemeFeedPage: FC = () => {
  ...
  const { isLoading, data: memes } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (let meme of memes) {
        const author = await Service.getUserById(token, meme.authorId);
        const comments: GetMemeCommentsResponse['results'] = [];
        const firstPage = await Service.getMemeComments(token, meme.id, 1);
        ...
        for (let i = 0; i < remainingCommentPages; i++) {
          const page = await Service.getMemeComments(token, meme.id, i + 2);
        ...
        for (let comment of comments) {
          const author = await Service.getUserById(token, comment.authorId);
  ...
  const { data: user } = useQuery({
    queryKey: ['user'],
    queryFn: async () => {
      return await Service.getUserById(token, jwtDecode<{ id: string }>(token).id);
    },
  });
  ...
  const { mutate } = useMutation({
    mutationFn: async (data: { memeId: string; content: string }) => {
      await Service.createMemeComment(token, data.memeId, data.content);
    },
  });
